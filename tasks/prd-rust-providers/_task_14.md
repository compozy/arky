...
- [ ] 14.1 Define `ArkyError` enum with `#[from]` conversions for all crate error types
- [ ] 14.2 Implement `ClassifiedError` for `ArkyError` by delegating to inner error variants
- [ ] 14.3 Create `prelude` module re-exporting commonly used types (Agent, AgentBuilder, Provider, Tool, etc.)
- [ ] 14.4 Re-export modules from all workspace crates under organized namespaces
- [ ] 14.5 Configure feature flags: `claude-code`, `codex`, `sqlite`, `server` for optional dependencies
- [ ] 14.6 Add crate-level documentation explaining the structure and re-exports
- [ ] 14.7 Write compile tests verifying that `use arky::prelude::*` provides expected types
- [ ] 14.8 Write tests verifying `ArkyError` conversions from all crate error types
...
- [ ] `ArkyError::from(CoreError)` conversion works
- [ ] `ArkyError::from(ProviderError)` conversion works
- [ ] `ArkyError::from(ToolError)` conversion works
- [ ] `ArkyError::from(SessionError)` conversion works
- [ ] `ArkyError::from(McpError)` conversion works
- [ ] `ArkyError::from(HookError)` conversion works
- [ ] `ArkyError::from(ConfigError)` conversion works
- [ ] `ArkyError` delegates `ClassifiedError` methods to inner variant correctly

### Integration Tests (Required)

- [ ] `use arky::prelude::*` compiles and provides: `Agent`, `AgentBuilder`, `Provider`, `Tool`, `ToolDescriptor`, `Message`, `AgentEvent`, `SessionStore`
- [ ] Feature flag combinations compile: default, `claude-code`, `codex`, `sqlite`, `server`, all features

### Regression and Anti-Pattern Guards

- [ ] No re-export conflicts (multiple types with same name from different crates)
- [ ] Feature flags properly gate optional dependencies
- [ ] Prelude does not re-export overly specific internal types
...
- [ ] `cargo test -p arky`
- [ ] `cargo test -p arky --all-features`
