//! # 05 Codex Tools
//!
//! Live, self-checking provider-native tool validation for Codex.
...
async fn main() -> Result<(), ExampleError> {
    let workspace = temporary_workspace("codex-tools")?;
    let model = codex_model();
    let token = "CODEX_TOOL_OK_445566";
    ...
    require_any_tool_execution(
        &events,
        "Codex tools example should observe a provider-native tool execution",
    )?;
    require_event(&events, "Codex should emit tool start", |event| {
        matches!(event, AgentEvent::ToolExecutionStart { .. })
    })?;

    let final_text = final_turn_text(&events)?;
    require_contains(&final_text, token, "Codex tool-assisted response")?;
    pass("Codex executed provider-native tools and returned the file token");
